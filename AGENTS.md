...
| `/jip` | `jip.md` | Hermes | Hertaal een technisch antwoord of rapport naar eenvoudige taal voor een volwassene. Regels in [[GL-023-eenvoudige-taal]]; getallen, paden en commando's blijven woordelijk exact. Varianten: `/jip <bestand>`, `/jip <vraag>`, `/jip diep`, `/jip aan` / `/jip uit` |
| `/caveman` | `caveman.md` | Hermes | Aan/uit-modus voor extreem beknopte output — geen inleidingen, geen samenvattingen, wel altijd woordelijk exacte code, paden en foutmeldingen. Voor lange debug-sessies en tokenbewust werk; niet voor klantcommunicatie, verslagen of journaling |
...
| `/wdf-regels` | `~/.claude/skills/wdf-regels/SKILL.md` | Daedalus | Vragen over de World Darts Federation beantwoorden, gegrond in de lokale kennisbasis `PKM/Documents/WDF-Kennis/` (16 kennisbestanden + de 15 originele PDF's in `bron/`): officiële spelregels, kwalificatiecriteria (World Championship + World Masters, senioren/jeugd), ranking-systeem (senioren/jeugd/U23), de zes Cup-reglementen, de deelnamegerechtigdheid uit de Bye-Laws (clausule 2.01 en 7.05 — alleen die laag, niet het hele reglement) en de organisatiestructuur inclusief het WDF-lidmaatschap van de Nederlandse Dartsbond. Nooit uit het geheugen antwoorden — altijd eerst het kennisbestand lezen; bij een hiaat de bron noemen of aanbieden na te zoeken. `INDEX.md` bevat een revisieregister om te detecteren of de WDF de reglementen inmiddels herzien heeft. Net als `/transcribeer` een globale Skill (`~/.claude/skills/`), dus per machine apart geïnstalleerd. Gebouwd in [[tsk-2026-08-21-002]]. |
...
1. `git pull` from `origin` first — this surfaces conflicts from another session's concurrent close-session (no locking exists between parallel chats) instead of letting the later push fail unexplained or, worse, diverge silently.
2. `git add -A` (stage everything, including new session-log, new notes, new images, edited files).
3. `git commit -m "Session backup YYYY-MM-DD HH:MM"` (use the actual close timestamp).
4. `git push` to `origin`.
5. Report the result to the user in one line (committed + pushed, or what failed and why).

If the repo has no remote configured, or pull/push fails (auth, conflict, offline), tell the user explicitly — never silently skip the backup. On a pull conflict, stop and show the user which files conflict rather than resolving it unilaterally. This step is not optional and does not require the user to ask for it each time; it is part of what "close session" means in this myPKA.
