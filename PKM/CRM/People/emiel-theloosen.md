---
full_name: Emiel Theloosen
first_name: Emiel
last_name: Theloosen
email: e.theloosen@calvi-insight.com
relation: professional
role: Bestuurslid (elect)
company: RDB (Rivierenlanddartsbond)
tags:
  - darts
  - rdb
  - bestuur
  - website
---

# Emiel Theloosen

## In één zin

Aankomend bestuurslid RDB, portefeuille communicatie — verantwoordelijk voor website, socials, tone of voice en sponsoring.

## Achtergrond

- Werkt bij **Calvi** (calvi-insight.com) als **Product Owner**
- Gestudeerd aan Hogeschool van Arnhem en Nijmegen (HAN) — Nijmegenaar
- LinkedIn: https://www.linkedin.com/in/emiel-theloosen-b26b1716/
- Heeft zelf ervaring met WordPress en add-ons bouwen (o.a. statistieken voor VanAllesWa-website)

### Calvi Insight
Telecom billing specialist, opgericht 2001, gevestigd in Tilburg. 51-200 medewerkers. Bouwen Bill Communication software voor telecomproviders (EBPP, expense management, VIP billing). Klanten: Vodafone, KPN, Atos. ISO27001 & ISO9001 gecertificeerd.

## Darts

- Team: **VanAllesWa** — Derde Divisie B, RDB seizoen 25-26
- Locatie: **Café Piet Huisman**, St. Jacobslaan 30, 6533 BT Nijmegen
- Rol in team: **Captain** (C-badge)
- Statistieken seizoen 25-26: 16 singles gespeeld, 68.8% winst
- Bijzonder resultaat: 120-finish
- Zelf: geniet meer van gezelligheid dan competitieniveau, middenmoot derde divisie

## RDB rol

- Aankomend bestuurslid RDB — officiële verkiezing op ALV **4 juli 2026**
- Portefeuille: **communicatie** — website, socials, sponsoring, tone of voice
- Door Danny aangewezen als enige verantwoordelijke voor communicatie (geen kruisbestuiving meer zoals onder Nick)
- Via Nick in contact gekomen met Sander — vraag over hostinggegevens huidige website

## Visie communicatie

- Simpele, openbare WordPress website — geen login walls, laagdrempelig
- Alles wat op Facebook staat ook op website, en vice versa
- Één stem/tone of voice naar buiten (niet meerdere schrijvers zichtbaar)
- Canva brand kit aanmaken (logo, kleuren, stijlen)
- Mogelijk later Instagram erbij
- Wil sponsoring oppakken: ZZP'ers en kleine bedrijfjes onder leden aanspreken, sponsorboard op toernooien/website
- Groeiambitie: bond naar 1200 leden

## Technische context (website)

- Hosting bij **Sander** via **WPMU Dev** (dedicated, premium plugins)
- E-mail via **Google Workspace** (oud gratis legacy account ~7-8 adressen; via Serge Taber destijds ingestapt)
- Domein gekoppeld aan hosting, mails via Google DNS
- Emiel wil een **tweede WordPress instantie** naast de bestaande opzetten (offline bouwen, dan live voor ALV)
- Sander stuurt inloggegevens WPMU Dev + Google woensdag 25 juni

## Afspraken & contact

- 2026-06-22 ~20:00 (maandagavond): kennismaking online — websitehosting RDB besproken ✓
- 2026-06-25 (woensdag): Sander stuurt inloggegevens hosting + Google naar **e.theloosen@calvi-insight.com**
- Wens: maandelijks sparren over website/communicatie

## Actiepunten

- [ ] **Sander → Emiel** (woensdag 25 juni): inloggegevens WPMU Dev dashboard + Google Workspace sturen
- [ ] **Sander**: Emiel link sturen naar WPMU Dev plugin-overzicht
- [ ] **Emiel**: na ALV (4 juli) website live met nieuwe bestuursinformatie + communicatievisie

## Open vragen

- ~~In welk RDB-team gooit hij precies?~~ → VanAllesWa, 3e divisie B
- ~~Wat heeft hij nodig?~~ → WPMU Dev + Google Workspace inloggegevens
- ~~Scope website?~~ → Simpele WordPress, alles openbaar, nieuws + bestuur + reglementen, later eventueel spelersmarktplaats
