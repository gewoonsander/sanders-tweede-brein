...
- **2026-07-19** — Ticket "E-mail instellen in Outlook" verder gediagnosticeerd. Heleen kan wél via haar witgelabelde klantportaal (gewoonsander.nl/hub) bij haar mailbox komen, maar niet via directe login (pro.yourwpsite.email) of Outlook. Verklaring: hub-toegang gaat via SSO (single sign-on, geen wachtwoord nodig), directe login/Outlook hebben wel het echte wachtwoord nodig. `pro.yourwpsite.email` is bevestigd de juiste, officiële directe-toegang-URL (WPMU Dev-documentatie) — geen link-probleem dus. Werkhypothese: het nieuwe wachtwoord dat Heleen op 14-07 zelf instelde, is niet goed doorgekomen op de mailserver. **Volgende stap: Sander controleert/reset het wachtwoord zelf aan de admin-kant in de Hub (Pro Email-beheerscherm), voordat er op het ticket gereageerd wordt.**
  - **Update:** wachtwoord is gereset aan de admin-kant in de Hub (2026-07-19). Concept-antwoord klaargezet in Gmail (nog niet verzonden) met het nieuwe wachtwoord en het verzoek om zowel de webmail-link als Outlook opnieuw te proberen.
