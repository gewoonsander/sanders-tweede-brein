...
- [ ] **Concept-factuur op het RDB-klantrecord bekijken en bevestigen** (hosting €420/jaar + 2x domeinverlenging €8/jaar, totaal €527,56 incl. btw) — pas na bevestiging wordt dit een actief, jaarlijks terugkerend abonnement zichtbaar voor Emiel. Deze factuur betreft het volledige jaar 2026 — de vorige factuur (2212) is op 9 december 2025 door RDB voldaan en betrof de hosting van 2025. **Betaling loopt via Sjoerd, de nieuwe penningmeester** — per creditcard als de bond die inmiddels heeft (vorige keer nog niet het geval), anders per bankoverschrijving o.v.v. het factuurnummer. Zodra betaald, zet Sander de factuur handmatig op "betaald" in de Hub.
...
- [ ] **Sanders eigen facturatiesysteem stopzetten voor hosting + domeinen van RDB** zodra de Hub-factuur bevestigd is, om dubbele facturatie te voorkomen.
- [x] **Inloggegevens van de huidige RDB-website (WordPress) toegevoegd aan het overdrachtsdocument** — URL `rivierenlanddartsbond.nl/wp-admin/`, gebruikersnaam `webmaster@rdbdarts.nl`, wachtwoord gemaskeerd (echte wachtwoord via apart kanaal, zelfde aanpak als bij het klantenportaal-wachtwoord).
- [ ] **`webmaster@rdbdarts.nl` overdragen aan Emiel** — pas nadat zijn Hub-toegang werkt (zie hieronder), om Sanders eigen SSO-toegang tot de mailbox niet voortijdig te breken.
- [ ] **WPMU Dev Hub-klantrol "Webmaster" voor Emiel opzetten** — uitgebreidere versie van het Heleen-sjabloon (rol "Klant - Mail en Facturering" + Pro Email): moet ook zicht geven op het dedicated-hostingproduct (€35/maand) en facturen, plus waarschijnlijk WordPress-rol Administrator i.p.v. Editor. **Sander voert dit zelf uit** — Hermes begeleidt stap voor stap, maar voert accounttoegang-wijzigingen niet zelf uit.
...
- [ ] Alle toegang en informatie voor Emiel regelen (WPMU Dev dashboard, Google Workspace, plugin-overzicht) — vergelijkbaar met de Heleen-aanpak maar breder, want bestuurslid-niveau — **urgent: vandaag nog, vóór vanavond**
