...
| **Habit heatmap / streaks** | `habit_logs` | `v_habit_heatmap`, `v_habit_streaks` | `habit_logs(habit_slug, log_date, done, amount, unit, log_schema)` joined to `habits.name` | core regen, derived from dated `## Reflection` check-ins in Habit markdown |
