...
Lees de canonieke taken uit `PKM/Tasks/` conform [[GL-019-persoonlijke-taakarchitectuur]]. Toon: `scheduled` voor vandaag, echte `due_date` vandaag of verlopen, beschikbare belangrijke `next`-acties en `waiting`-taken waarvan `follow_up_date` vandaag of verlopen is.

**Verplichte Todoist-crosscheck (niet overslaan, ook niet als de PKM-bestanden er compleet uitzien):** roep `find-tasks` aan met filter `today | overdue`. Vergelijk elk resultaat met de canonieke PKM-bestanden op twee manieren:
- **Datum-mismatch:** een taak met een `todoist_id` waarvan de Todoist-datum afwijkt van de `scheduled_date`/`due_date` in het PKM-bestand — dit betekent dat Sander 'm in Todoist heeft verplaatst. Todoist is dan de meest recente intentie; werk het PKM-bestand bij om weer gelijk te trekken (met een Geschiedenis-regel), meld het, ga niet stilzwijgend uit van de oude PKM-datum.
- **Geen canonieke bron:** een Todoist-taak zonder duidelijke koppeling naar een `PKM/Tasks/`-bestand (geen "Canonieke taak: ..."-regel in de beschrijving, geen matchende `todoist_id` ergens in `PKM/Tasks/`). Dit is een taak die nooit is gecanoniseerd — meld 'm apart en canoniseer 'm dezelfde sessie (nieuw bestand volgens GL-019/SOP-022) in plaats van 'm te laten voortbestaan als PKM-onzichtbaar.

Gebruik Eisenhower voor rangschikking en groepeer per Project of Key Element bij meer dan 5 taken. Als er niets staat (in zowel PKM als Todoist): meld dat expliciet.
